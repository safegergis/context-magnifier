...
class TransparentWindow(QMainWindow):
    def __init__(self):
        ...
        font_path = os.path.join(
            os.path.dirname(__file__), "fonts", "KdamThmorPro-Regular.ttf"
        )
        ...
        top_widget = QSvgWidget(
            os.path.join(os.path.dirname(__file__), "assets", "top.svg")
        )
        top_widget.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)

        bottom_widget = QSvgWidget(
            os.path.join(os.path.dirname(__file__), "assets", "bottom.svg")
        )
        ...
        SettingWidget("grid x (default 16)")
        SettingWidget("grid y (default 9)")
        SettingWidget("base size (default 20)")
        SettingWidget("max size factor (default 4)")
        SettingWidget("min size factor (default 1)")
        SettingWidget("confidence threshold (default 20)")
        SettingWidget("button importance (default 3)")
        SettingWidget("input field importance (default 2)")
        SettingWidget("checkbox importance (default 1)")
        SettingWidget("confirmation importance (default 3)")
        SettingWidget("error importance (default 2.5)")
        SettingWidget("title importance (default 1.5)")
        SettingWidget("length importance (default 1.5)")
        SettingWidget("density importance (default 0.2)")
        SettingWidget("magnification (default 2.5)")
        ...
        grid_layout.setAlignment(close_button, Qt.AlignmentFlag.AlignHCenter)
